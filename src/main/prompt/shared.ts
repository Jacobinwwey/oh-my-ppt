...
export const FRONTEND_CAPABILITIES = [
  ...
  '动画默认使用 data-anim 声明式属性，不要为普通入场动画、逐条展示动画编写 <script> 或自定义 JS。',
  '简单动画优先使用 data-anim 属性声明，无需编写 JavaScript：',
  '```html',
  '<!-- 单个元素：页面加载时淡入上滑 -->',
  ...
  '当用户提到"演讲节奏、逐步讲解、逐条展示、点击出现、逐项展开、按键展示、present mode"时，优先使用 data-anim-trigger="click"。',
  '只有在 data-anim 无法表达的复杂时间线、自定义回调、复杂同步编排场景，才允许使用 <script> + PPT.animate() / createTimeline() / stagger()。',
  '禁止直接调用 anime(...) 或 anime.*。禁止为了动画初始态在 CSS/class/style 中写 opacity:0、visibility:hidden、display:none。',
  '',
